...
class Model(nn.Module):
    def __init__(self,config,load_emb=False,path=None,bias=True):
        ...
        #self.init_tree()
        # model_name = "/home/ubuntu/Suzuka_models/120M"
        model_name = config.draftpath
        draft_config = AutoConfig.from_pretrained(model_name)
        self.draft_model = AutoModelForCausalLM.from_pretrained(model_name,  device_map="auto", torch_dtype=torch.float16)
        # self.layers = nn.ModuleList([LlamaDecoderLayer(config,index) for index in range(config.num_hidden_layers)])

        self.fc=nn.Linear(2*config.hidden_size, draft_config.hidden_size,bias=bias)
        self.fc1=nn.Linear(draft_config.hidden_size, config.hidden_size, bias=bias)
